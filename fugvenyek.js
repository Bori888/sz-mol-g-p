...
export function GombokLetrehozasa() {
    const GOMBOK=document.querySelector(".szamok")
    for (let index = 0; index < 10; index++) {
        GOMBOK.innerHTML += `<button>${index}</button>`
    }
...
export function SzamokMegjelenit() {
...
export function muvJelek() {
    ...
    function evtKreal(valtozo, jel) {
      valtozo.addEventListener("click", function () {
        ...
        if (
            //INNENTÖL NEM ÉRTEM  CSAK MÁSOLOK
            !(
